...
@Controller()
public class BeerRecordController {
	 ...
	 * 查看啤酒记录信息
	 */
	@RequestMapping("/m_beerrecordlist")
	public String m_beerrecordlist(HttpServletRequest request,HttpServletResponse response) throws UnsupportedEncodingException{
		...
		List<BeerRecord> cuns=beerrecordService.selectBy(max, min, name, tele);
		request.setAttribute("list", cuns);
		return "m_beerrecordlist";
	}
	
	@ResponseBody
	@RequestMapping("/downBR")
	public void down(String id,HttpServletRequest request) throws NumberFormatException, UnsupportedEncodingException{
		int i=Integer.valueOf(id);
		List<BeerRecord> beerRecords=beerrecordService.selectBy(null, null, null, null);
		Iterator<BeerRecord> usIterator=beerRecords.iterator();
		...
		for(int i1=1;i1<8;i1++){
			...
			while(usIterator.hasNext()){
				BeerRecord beerRecord=usIterator.next();
				...
				m.put(4, beerRecord.getRule_name());
				m.put(5, beerRecord.getPay());
				m.put(6, beerRecord.getUser_old());
				m.put(7, beerRecord.getUser_num());
				m.put(8, beerRecord.getUser_new());
				m.put(9, beerRecord.getManager());
			}
			datas.add(m);
		}
